...
class UserController extends AbstractController
{
    ...
    public function editUserAction()
    {
        ...
        $this->data['uTrueValues'] = $model->select_it(null, self::TBL_USER, ['mobile', 'image', 'province', 'city'], 'id=:id', ['id' => $this->data['identity']->id])[0];
    ...
    public function userDepositAction()
    {
        ...
        try {
            $form->beforeCheckCallback(function (&$values) use ($model, $form) {
                ...
                $res = $this->_chargeAccountProcess((int)$values['price']);
                if (!$res) {
                    $form->setError('خطا در ارتباط با درگاه بانک! لطفا دوباره سعی کنید.');
                }
            });
        } catch (Exception $e) {
            die($e->getMessage());
        ...
        try {
            ...
            if (isset($postVars['order_id']) && isset($postVars['status']) &&
                ...
                if (count($orderPayment) == 1) {
                    ...
                    if (!in_array($orderPayment['status'], [Payment::PAYMENT_STATUS_OK_IDPAY, Payment::PAYMENT_STATUS_DUPLICATE_IDPAY]) &&
