...
class StringConverter
{
    ...
    const CONVERT_ALL = self::CONVERT_NUMBERS | self::CONVERT_CHARACTERS;
     ...
     * @return array|mixed
     */
    public static function toPersian($str, $convert_type = self::CONVERT_ALL)
    {
        if (self::CONVERT_NUMBERS & $convert_type || self::CONVERT_ALL & $convert_type) {
            if (is_array($str)) {
                $newArr = [];
                foreach ($str as $k => $v) {
        ...
        } elseif (self::CONVERT_CHARACTERS & $convert_type || self::CONVERT_ALL & $convert_type) {
            if (is_array($str)) {
                $newArr = [];
                foreach ($str as $k => $v) {
     ...
     * @return array|mixed
     */
    public static function toArabic($str, $convert_type = self::CONVERT_ALL)
    {
        if (self::CONVERT_NUMBERS & $convert_type || self::CONVERT_ALL & $convert_type) {
            if (is_array($str)) {
                $newArr = [];
